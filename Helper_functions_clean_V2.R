...
## Function body ===============================================================  
  ...
  # drop the first row, which is not followed by any camera 
...
getTimes <- function(sp1, sp2, data, interval = 0, rcens = NA,
                     rtrunc = NA, excludecen = FALSE, keepends = FALSE){
...
# keepends: if true, the function will add a row for with the time_difference 
# between the last observation of a species of interest (sp1 or sp2), based on
# the last picture taken by the camera in the dataset (does not have to be sp1 
# or sp2)
...
# times <- getTimes("coyote", "fox", sdata1, 60)
# times <- getTimes("coyote", "fox", sdata1, 10, rcens  = 40, excludecen = TRUE)
...
## Function body ===============================================================  
  ...
  data <- controlInterval(data, interval, keep = c(sp1, sp2))
  
  locale = locale(date_format = "%Y-%m-%d",
                  time_format = "%H:%M:%S",
                  tz = "UTC")
  
  subdata <- data %>% mutate(Time_diff = lead(DateTime) - DateTime,
                             Next_sp = lead(Species), 
                             DateTime_Next_sp = lead(DateTime),
                             DateTime_Next_sp = lead(DateTime),
                             RelativePath_Next_sp = lead(RelativePath))
  
  if (keepends == TRUE){ 
    subdata <- subdata %>%
      rowwise() %>%
      # if a row corresponds to the last species detected by the camera, the next species is marked with end
      mutate(Next_sp = ifelse(RelativePath != RelativePath_Next_sp, "end",
                              Next_sp))
    
    # assign end for the Next_sp of the last row in the dataset
    subdata$Next_sp[nrow(subdata)] <- "end"
    
    # The actual timing of the end of the camera trap period is unknown, so 
    # instead we will use the time of the last detection in the dataset
    # this does not have to be sp1 or sp2, but it must be a species that is 
    # present in the data used for the parameters
    for (g in unique(subdata$RelativePath)){
      last_time_interest <- tail(subdata[(subdata$RelativePath == g),]$DateTime, n = 1) 
      last_time_dataset <- tail(data_org[(data_org$RelativePath == g),]$DateTime, n = 1) 
      
      num_detections <- length(subdata[(subdata$RelativePath == g),]$DateTime)
      
      subdata[(subdata$RelativePath == g),]$Time_diff[num_detections] <- last_time_dataset - last_time_interest
      subdata[(subdata$RelativePath == g),]$DateTime_Next_sp[num_detections] <- last_time_dataset 
      subdata[(subdata$RelativePath == g),]$RelativePath_Next_sp[num_detections] <- g 
    ...
    #remove all rows that correspond to the last observation of a species of interest 
    #before the end of the camera traping period
    subdata <- data.frame(subdata)
    subdata <- filter(subdata, subdata$Next_sp != "end")
  }
  
  ## convert from difftime values in the Time_diff column to hours
  subdata$Time_diff <- as.numeric(subdata$Time_diff, units = "hours")
  
  # add a column for censor times. This column displays a 0 when the next species
  # to be detected was sp1, or if the camera trapping period ended (censored times)
  # otherwise, it displays a 0 
  subdata <- subdata %>%
    mutate(Censor = ifelse(Next_sp %in% c(sp1, "end"), 0,1)) 
  ...
  if (is.na(rcens) == FALSE){
    subdata <- subdata %>%
      mutate(Censor = ifelse(subdata$Time_diff > rcens, 0, Censor))
  }
  
  subdata <- data.frame(subdata)
  
  #if rtrunc was provided, remove all observations with a time interval above this value 
  if (is.na(rtrunc) == FALSE){
    subdata <- filter(subdata, subdata$Time_diff < rtrunc)
  }
  
  #if excludecen is true, remove all censored observations
  if (excludecen == TRUE){
    subdata <- filter(subdata, subdata$Censor != 0)
  }
  
  ##Keep only observations when sp1 is the one that is observed 
  subdata <- subdata[(subdata$Species == sp1),]
  ...
  subdata <- subdata[(subdata$Time_diff > (interval/60)),]
...
addFreqSp <- function(data, sp){
  ...
  #add rows with zeroes for both sites and months where the species was not detected
  Weekdf <-complete(Weekdf, Species, RelativePath, WeekIndex, fill = list(Nspecies = 0)) 
  ...
  #add rows with zeroes for both sites and months where the species was not detected
  Monthdf <- complete(Monthdf, Species, RelativePath, MonthIndex, fill = list(Nspecies = 0))
  ...
  for (i in 1:length(sp)){
    ...
    Weekfreqs <- Weekdf[(Weekdf$Species == sp[i]),] 
    Monthfreqs <- Monthdf[(Monthdf$Species == sp[i]),]
